...
export const ApiKeyManager: React.FC<ApiKeyManagerProps> = ({ onSaveKey, onRemoveKey, isKeySet, currentKeySource }) => {
  const [apiKeyInput, setApiKeyInput] = useState<string>('');
  const [showSavedMessage, setShowSavedMessage] = useState<boolean>(false);
  const [isExpanded, setIsExpanded] = useState<boolean>(!isKeySet);

  // Fix: The previous logic in this hook was overly complex, contained dead code,
  // and caused a TypeScript error because `import.meta.env` was not correctly typed.
  // The simplified hook now correctly sets the expansion state based on whether an
  // API key is set, which maintains the intended functionality while fixing the error.
  useEffect(() => {
